...
pro.send = function (reqId, route, msg, recvs, opts, cb) {
    ...
    var sentPacket = this.app.get('sentPacket');
    var hourSent = sentPacket[hour];
    if(hourSent){
        if(hourSent[route]){
            hourSent[route].count += 1;
            hourSent[route].bytes += emsg.length;
        }else{
            var pack = {};
            pack.count = 1;
            pack.bytes = emsg.length;
            hourSent[route] = pack;
        }
    } else{
        var data = {};
        data[route] = {count :1,bytes:emsg.length};
        sentPacket[hour] = data;
    }
...
var bindEvents = function (self, socket) {
    ...
    var receivedPacket = {};
    var sentPacket = {};
    self.app.set('receivedPacket',receivedPacket);
    self.app.set('sentPacket',sentPacket);
    ...
    socket.on('message', function (msg) {
        ...
        var nowDate = new Date();
        var hour =  nowDate.getHours();
        var receivedPacket = self.app.get('receivedPacket');
        var hourRecv = receivedPacket[hour];
        if(hourRecv){
            if(hourRecv[dmsg.route]){
                hourRecv[dmsg.route].count += 1;
                hourRecv[dmsg.route].bytes = +msg.body.length;
            }else{
                var pack = {};
                pack.count = 1;
                pack.bytes = msg.body.length;
                hourRecv[dmsg.route] = pack;
            }
        } else{
            var data = {};
            data[dmsg.route] = {count :1,bytes:msg.body.length};
            receivedPacket[hour] = data;
        }
